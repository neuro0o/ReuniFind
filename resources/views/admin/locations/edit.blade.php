...
<div class="layout">
    ...
    <div class="content">
        <div class="form-card">
            ...
            <form action="{{ route('locations.update', $location->locationID) }}" method="POST">
                @csrf
                @method('PUT')

                <label for="locationName">Location Name:</label>
                <input type="text" name="locationName" id="locationName" value="{{ old('locationName', $location->locationName) }}" required>
                @error('locationName') <small style="color:red">{{ $message }}</small> @enderror
                <br>

                <label for="latitude">Latitude:</label>
                <input type="number" step="0.000001" name="latitude" id="latitude" value="{{ old('latitude', $location->latitude) }}" required>
                @error('latitude') <small style="color:red">{{ $message }}</small> @enderror
                <br>

                <label for="longitude">Longitude:</label>
                <input type="number" step="0.000001" name="longitude" id="longitude" value="{{ old('longitude', $location->longitude) }}" required>
                @error('longitude') <small style="color:red">{{ $message }}</small> @enderror
                <br>

                <div class="button-stack">
                    <button type="submit" class="btn btn-primary" id="update-location-btn">Update Location</button>
